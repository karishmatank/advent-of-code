...
# Given all connections, create circuits
# We'll represent a circuit by the unique boxes that make up that circuit
connections = get_sorted_connections()
...
for connection in connections:
    ...
    # If we only have one circuit, and that circuit has all of the boxes already, stop iterating
    # (circuits are sets, so len of circuit will match len of boxes list if so)
    if len(circuits) == 1 and len(circuits[0]) == len(data):
        # print("Current circuit already has all boxes!")
        break

# `connection` variable represents last pair connected together to connect all boxes into one circuit
# This is because we break at the end of the loop body
# For this connection, we need to get the x values and multiply them
x_vals = list(zip(*connection))[0]
print(calc_product(x_vals)) # Part 2 correct